...
plot.NMixPredCondDensJoint2 <- function(x, ixcond, imargin, contour=FALSE, auto.layout=TRUE, col, lwd=1, main, xylab, ...)
{
  ...
  ##### Plot of one bivariate conditional density given (in a sequel) all values of the margin we have conditioned
  ##### -------------------------------------------------------------------------------------------------------------
  if (!missing(imargin)){
    ...
    for (i in 1:length(xcond)){
      if (imargin[1] > imargin[2]) dx <- t(x$dens[[i]][[NAAM]])
      else                         dx <- x$dens[[i]][[NAAM]]
