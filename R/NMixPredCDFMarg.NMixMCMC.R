...
## *************************************************************
NMixPredCDFMarg.NMixMCMC <- function(x, grid, lgrid=50, scaled=FALSE, ...)
{
  if (missing(grid)){
    grid <- list()
    if (scaled){
  ...
  if (x$prior$priorK == "fixed"){
    return(NMixPredCDFMarg.default(x=grid, scale=scale, K=x$K, w=as.numeric(t(x$w)), mu=as.numeric(t(x$mu)), Li=as.numeric(t(x$Li)), Krandom=FALSE))
  }else{
    return(NMixPredCDFMarg.default(x=grid, scale=scale, K=x$K, w=x$w, mu=x$mu, Li=x$Li, Krandom=TRUE))
  }    
}  
